...
def Matrix_trasformation(i, w, Omega):
    Matrix = np.zeros((3, 3))
...
def vec_xyz(a, e, E):
    ...
    Vec[0, 0] = a*(np.cos(E) - e)
    Vec[1, 0] = a*np.sin(E)*sqrt_e(e)
    Vec[2, 0] = 0
...
def vec_vxyz(a, e, E):
    ...
    Vec[0, 0] = -factor*np.sin(E)
    Vec[1, 0] = factor*sqrt_e(e)*np.cos(E)
    Vec[2, 0] = 0
...
def a_xyz(e, E):
    ...
    Vec[0, 0] = np.cos(E) - e
    Vec[1, 0] = np.sin(E)*sqrt_e(e)
    Vec[2, 0] = 0

    return Vec

def a_vxvyvz(a, e, E):
    factor = nu(a)/(a*r(a,e,E))

    Vec[0, 0] = factor*np.sin(E)
    Vec[1, 0] = factor*np.cos(E)
    Vec[2, 0] = 0

    return Vec

#----------------Derivates respecto to e----------------

def e_xyz(a, e, E):
    factor = a*np.sin(E)

    Vec[0, 0] = factor * a/r(a, e, E) * np.sin(E) + 1
    Vec[1, 0] = factor * e/sqrt_e(e) - a/r(a, e, E) * (sqrt_e(e) * np.cos(E))
    Vec[2, 0] = 0
...
def e_vxvyvz(a, e, E):
    ...
    Vec[0, 0] = nu(a)*a*np.sin(E)/(r(a, e, E)**2) * (2*np.cos(E) - a*e*np.sin(E)**2/r(a, e, E))
    Vec[1, 0] = -nu(a)*e*np.cos(E)/(r(a, e, E)*sqrt_e(e)) + nu(a)*a*sqrt_e(e)/(r(a, e, E)**2) * (1 - 2*np.sin(E)**2 - a*e*np.cos(E)*np.sin(E)**2/r(a, e, E))
    Vec[2, 0] = 0

    return Vec

#----------------Derivates respecto to i----------------

def i_xyz(a, e, i, Omega, w, E):
    xyz = Matrix_trasformation(Omega, w, i)@vec_xyz(a, e, E)
    
    Vec[0, 0] = xyz[2,0] * np.sin(Omega)
    Vec[1, 0] = -xyz[2,0] * np.cos(Omega)
    Vec[2, 0] = -xyz[0,0] * np.sin(Omega) + xyz[1,0] * np.cos(Omega)

    return Vec

def i_vxvyvz(a, e, i, Omega, w, E):
    vxyz = Matrix_trasformation(Omega, w, i)@vec_vxyz(a, e, E)

    Vec[0, 0] = vxyz[0,0] * np.sin(Omega)
    Vec[1, 0] = -vxyz[0,0] * np.cos(Omega)
    Vec[2, 0] = -vxyz[2,0] * np.sin(Omega) + vxyz[1,0] * np.cos(Omega)

    return Vec

#----------------Derivates respecto to Omega----------------

def Omega_xyz(a, e, i, Omega, w, E):
    xyz = Matrix_trasformation(Omega, w, i)@vec_xyz(a, e, E)

    Vec[0, 0] = -xyz[1,0] * np.sin(Omega)
    Vec[1, 0] = -xyz[0,0] * np.cos(Omega)
    Vec[2, 0] = 0

    return Vec

def Omega_vxvyvz(a, e, i, Omega, w, E): 
    vxyz = Matrix_trasformation(Omega, w, i)@vec_vxyz(a, e, E)

    Vec[0, 0] = -vxyz[1,0] * np.sin(Omega)
    Vec[1, 0] = vxyz[2,0] * np.cos(Omega)
    Vec[2, 0] = -vxyz[2,0] * np.sin(Omega) + vxyz[1,0] * np.cos(Omega)

    return Vec

#----------------Derivates respecto to w----------------

def w_xyz(a, e, i, Omega, w, E):
    xyz = Matrix_trasformation(Omega, w, i)@vec_xyz(a, e, E)

    Vec[0, 0] = -xyz[1,0]*np.cos(i) - xyz[2,0]*np.sin(i)*np.cos(Omega)*np.sin(Omega)
    Vec[1, 0] = xyz[0,0]*np.cos(i) - xyz[2,0]*np.sin(i)*np.sin(Omega)
    Vec[2, 0] = xyz[0,0]*np.sin(i)*np.cos(Omega) + xyz[1,0]*np.sin(i)*np.sin(Omega)

    return Vec

def w_vxvyvz(a, e, i, Omega, w, E): 
    vxyz = Matrix_trasformation(Omega, w, i)@vec_vxyz(a, e, E)

    Vec[0, 0] = -vxyz[1,0]*np.cos(i) - vxyz[0,0]*np.sin(i)*np.cos(Omega)
    Vec[1, 0] = vxyz[2,0]*np.cos(i) - vxyz[2,0]*np.sin(i)*np.sin(Omega)
    Vec[2, 0] = vxyz[2,0]*np.cos(Omega)*np.sin(i) + vxyz[1,0]*np.sin(Omega)*np.sin(i)

    return Vec

#----------------Derivates respecto to M----------------

def M_xyz(a, e, i, Omega, w, E):
    vxyz = Matrix_trasformation(Omega, w, i)@vec_vxyz(a, e, E)
    n = (mu/a**3)**0.5
...
def M_vxvyvz(a, e, i, Omega, w, E): 
    xyz = Matrix_trasformation(Omega, w, i)@vec_xyz(a, e, E)
    factor = -(mu*a**3)**0.5/r(a,e,E)**3

    Vec[0, 0] = factor*xyz[0,0]
    Vec[1, 0] = factor*xyz[1,0] 
    Vec[2, 0] = factor*xyz[2,0] 

    return Vec

def Jacobian_xE2(a, e, i, w, Omega, E):
    """
    Constructs the Jacobian matrix for the transformation between orbital elements
    and Cartesian state vector.
    ...
    partial_axyz = Matrix_trasformation(Omega, w, i)@a_xyz(E, e)
    partial_avxyz = Matrix_trasformation(Omega, w, i)@a_vxvyvz(E, e, a)

    partial_exyz = Matrix_trasformation(Omega, w, i)@e_xyz(E, e, a)
    partial_evxyz = Matrix_trasformation(Omega, w, i)@e_vxvyvz(E, e, a)

    partial_ixyz = Matrix_trasformation(i, w, Omega)@i_xyz(a, e, i, Omega, w, E)
    partial_ivxyz = Matrix_trasformation(i, w, Omega)@i_vxvyvz(a, e, i, Omega, w, E)

    partial_Omegaxyz = Matrix_trasformation(i, w, Omega)@Omega_xyz(a, e, i, Omega, w, E)
    partial_Omegavxyz = Matrix_trasformation(i, w, Omega)@Omega_vxvyvz(a, e, i, Omega, w, E)

    partial_wxyz = Matrix_trasformation(i, w, Omega)@w_xyz(a, e, i, Omega, w, E)
    partial_wvxyz = Matrix_trasformation(i, w, Omega)@w_vxvyvz(a, e, i, Omega, w, E)

    partial_Mxyz = Matrix_trasformation(i, w, Omega)@M_xyz(a, e, i, Omega, w, E)
    partial_Mvxyz = Matrix_trasformation(i, w, Omega)@M_vxvyvz(a, e, i, Omega, w, E)
    
    # First row: Partial derivatives of x with respect to orbital elements
    jacobian[0, 0] = partial_axyz[0,0]
    jacobian[0, 1] = partial_exyz[0,0]
    jacobian[0, 2] = partial_ixyz[0,0]
    jacobian[0, 3] = partial_Omegaxyz[0,0]
    jacobian[0, 4] = partial_wxyz[0,0]
    jacobian[0, 5] = partial_Mxyz[0,0]
    
    # Second row: Partial derivatives of y with respect to orbital elements
    jacobian[1, 0] = partial_axyz[1,0]
    jacobian[1, 1] = partial_exyz[1,0]
    jacobian[1, 2] = partial_ixyz[1,0]
    jacobian[1, 3] = partial_Omegaxyz[1,0]
    jacobian[1, 4] = partial_wxyz[1,0]
    jacobian[1, 5] = partial_Mxyz[1,0]
    
    # Third row: Partial derivatives of z with respect to orbital elements
    jacobian[2, 0] = partial_axyz[2,0]
    jacobian[2, 1] = partial_exyz[2,0]
    jacobian[2, 2] = partial_ixyz[2,0]
    jacobian[2, 3] = partial_Omegaxyz[2,0]
    jacobian[2, 4] = partial_wxyz[2,0]
    jacobian[2, 5] = partial_Mxyz[2,0]
    
    # Fourth row: Partial derivatives of vx with respect to orbital elements
    jacobian[3, 0] = partial_avxyz[0,0]
    jacobian[3, 1] = partial_evxyz[0,0]
    jacobian[3, 2] = partial_ivxyz[0,0]
    jacobian[3, 3] = partial_Omegavxyz[0,0]
    jacobian[3, 4] = partial_wvxyz[0,0]
    jacobian[3, 5] = partial_Mvxyz[0,0]
    
    # Fifth row: Partial derivatives of vy with respect to orbital elements
    jacobian[4, 0] = partial_avxyz[1,0]
    jacobian[4, 1] = partial_evxyz[1,0]
    jacobian[4, 2] = partial_ivxyz[1,0]
    jacobian[4, 3] = partial_Omegavxyz[1,0]
    jacobian[4, 4] = partial_wvxyz[1,0]
    jacobian[4, 5] = partial_Mvxyz[1,0]
    
    # Sixth row: Partial derivatives of vz with respect to orbital elements
    jacobian[5, 0] = partial_avxyz[2,0]
    jacobian[5, 1] = partial_evxyz[2,0]
    jacobian[5, 2] = partial_ivxyz[2,0]
    jacobian[5, 3] = partial_Omegavxyz[2,0]
    jacobian[5, 4] = partial_wvxyz[2,0]
    jacobian[5, 5] = partial_Mvxyz[2,0]
    
    return jacobian


def Jacobian_Ex2(a, e, i, w, Omega, E):
    return np.linalg.inv(Jacobian_xE2(a, e, i, w, Omega, E))